...
import org.ippul.infinispan.example.model.ObjectStoredInCache2;
...
@ApplicationScoped
public class CacheManager2 extends CacheProxy<ObjectStoredInCache2> {

    @Override
    public String getProtoName() {
        return "ObjectStoredInCache2";
    }
    ...
    protected void preload(Integer entryToLoad) {
        try(Jsonb jsonb = JsonbBuilder.create()) {
            List<ObjectStoredInCache2> data = jsonb.fromJson(loadDummyData(), new ArrayList<ObjectStoredInCache2>(){}.getClass().getGenericSuperclass());
            data.stream()
                    .limit(entryToLoad)
