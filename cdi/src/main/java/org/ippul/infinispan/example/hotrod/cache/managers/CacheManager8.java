...
import org.ippul.infinispan.example.model.ObjectStoredInCache8;
...
@ApplicationScoped
public class CacheManager8 extends CacheProxy<ObjectStoredInCache8> {

    @Override
    public String getProtoName() {
        return "ObjectStoredInCache8";
    }
    ...
    protected void preload(Integer entryToLoad) {
        try(Jsonb jsonb = JsonbBuilder.create()) {
            List<ObjectStoredInCache8> data = jsonb.fromJson(loadDummyData(), new ArrayList<ObjectStoredInCache8>(){}.getClass().getGenericSuperclass());
            data.stream()
                    .limit(entryToLoad)
