...
import org.ippul.infinispan.example.model.ObjectStoredInCache1;
...
@ApplicationScoped
public class CacheManager1 extends CacheProxy<ObjectStoredInCache1> {

    @Override
    public String getProtoName() {
        return "ObjectStoredInCache1";
    }
    ...
    protected void preload(Integer entryToLoad) {
        try(Jsonb jsonb = JsonbBuilder.create()) {
            List<ObjectStoredInCache1> data = jsonb.fromJson(loadDummyData(), new ArrayList<ObjectStoredInCache1>(){}.getClass().getGenericSuperclass());
            data.stream()
                    .limit(entryToLoad)
