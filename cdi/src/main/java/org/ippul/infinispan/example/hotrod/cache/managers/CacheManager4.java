...
import org.ippul.infinispan.example.model.ObjectStoredInCache4;
...
@ApplicationScoped
public class CacheManager4 extends CacheProxy<ObjectStoredInCache4> {
    ...
    protected void preload(Integer entryToLoad) {
        try(Jsonb jsonb = JsonbBuilder.create()) {
            List<ObjectStoredInCache4> data = jsonb.fromJson(loadDummyData(), new ArrayList<ObjectStoredInCache4>(){}.getClass().getGenericSuperclass());
            data.stream()
                    .limit(entryToLoad)
